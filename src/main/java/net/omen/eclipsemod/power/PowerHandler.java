...
import net.omen.eclipsemod.power.powers.TestingPower;
...
public class PowerHandler {
    ...
    static {
        ...
        REGISTERED_POWERS.add(TestingPower.class);
    }
    ...
    public static List<Ability> getPowerAbilities(ServerPlayer player) {
        String powerName = getPlayerPowerName(player);
        for (Class<? extends Power> powerClass : REGISTERED_POWERS) {
            try {
                Power power = powerClass.newInstance();
                if (power.getName().equals(powerName)) {
                    return power.getAbilities();
                }
            } catch (InstantiationException | IllegalAccessException e) {
                e.printStackTrace();
            }
    ...
    public static void executeAbility(ServerPlayer player, int abilityIndex) {
        List<Ability> abilities = getPowerAbilities(player);
        if (abilityIndex < abilities.size()) {
            abilities.get(abilityIndex).execute(player);
        }
    }
